...
(async () => {
  ...
  app.get( "/cars/:id",
    async ( req: Request, res: Response ) => {
      // params is reserved variable in Express
      let { id } = req.params;

      if (!id) {
        return res.status(400).send(`Paramter "id" is required.`);
      }
      
      // try to find the car with the requested id
  ...
  app.post( "/cars/",
    async ( req: Request, res: Response ) => {
      ...
      if (!make || !type || !model || !cost || !id) {
        return res.status(400).send(`Paramters required: make, type, model, cost, id.`);
      }
